#!/bin/sh
# Runs every check that must pass before shipping the Indonesian pack/site:
#   1. packbuilder check         - pack-level schema/coverage/article checks
#      (engine/tools/packbuilder/qa/check.py with the Indonesian spec)
...
PYTHONPATH="${PACKBUILDER_PATH:-engine/tools}" python3 -m packbuilder check --lang id --repo .
...
TMP="$(mktemp /tmp/indonesian_index_check.XXXXXX.html)"
trap 'rm -f "$TMP"' EXIT
